<template>
  <div class="ai-announcement-generator">
        ...
        <el-form ref="formRef" :model="formData" label-width="100px">
          ...
          <el-form-item label="文体风格" prop="style">
            <el-select 
              ...
              style="width: 100%"
            >
              <el-option label="热情激昂" value="enthusiastic" />
              <el-option label="正式严谨" value="formal" />
              <el-option label="温馨友好" value="friendly" />
              <el-option label="简洁明了" value="concise" />
              <el-option label="活泼生动" value="lively" />
              <el-option label="权威专业" value="authoritative" />
            </el-select>
          </el-form-item>

          <el-form-item label="预期效果" prop="expection">
            <el-input
              v-model="formData.expection"
              placeholder="例如：吸引更多人参与活动、激发读者热情、提高关注度等"
              maxlength="100"
              show-word-limit
            />
            <div class="input-tip">
              <el-icon><InfoFilled /></el-icon>
              描述您希望通过这个公告达到的效果，AI将针对性地生成内容
            </div>
          </el-form-item>
        </el-form>
...
import { checkAIStatus, generateAnnouncementContent, type AnnouncementGenerationRequest } from '@/api/ai'
...
const generateAnnouncement = async () => {
  ...
  try {
    const requestData: AnnouncementGenerationRequest = {
      content: formData.content || undefined,
      style: formData.style || undefined,
      expection: formData.expection || undefined
    }
    
    console.log('开始AI公告内容生成，请求数据:', requestData)
    
    const result = await generateAnnouncementContent(requestData)
    generatedText.value = result.generated_text
    
    ElMessage.success('AI公告内容生成完成')
    
  } catch (error) {
    console.error('AI公告内容生成失败:', error)
    errorMessage.value = 'AI公告内容生成失败: ' + (error instanceof Error ? error.message : '未知错误')
    ElMessage.error('AI公告内容生成失败，请稍后重试')
  } finally {
...
const applyToForm = () => {
  ...
  ElMessage.success('AI生成的公告已应用到表单')
}
